...
$this->Ln(30);
		...
		$this->SetX($left += 30); $this->Cell(60, $h, 'TGL KELUAR', 1, 0, 'C',true);
		$this->SetX($left += 60); $this->Cell(80, $h, 'MAC ADDRESS', 1, 0, 'C',true);
		...
		$this->SetWidths(array(30,60,80,200,80,150,70,90,60,60));
		$this->SetAligns(array('C','C','L','L','L','L','L','L','L','L'));
...
$query="select barang_keluar.*, detil_barang.*, barang.* , jenis_barang.* 
from barang_keluar, detil_barang,barang, jenis_barang 
where barang_keluar.id_detil=detil_barang.id_detil and detil_barang.status='out' and jenis_barang.id_jenis=barang.jenis and
detil_barang.id_barang=barang.id_barang and 
$kategori LIKE'%$keyword%' and tgl_instalasi between '$tanggal_awal' and '$tanggal_akhir' order by barang_keluar.tgl_instalasi asc";	
}
else {
$query="select barang_keluar.*, detil_barang.*, barang.* , jenis_barang.* 
from barang_keluar, detil_barang,barang, jenis_barang 
where barang_keluar.id_detil=detil_barang.id_detil and detil_barang.status='out' and jenis_barang.id_jenis=barang.jenis and
detil_barang.id_barang=barang.id_barang and 
tgl_instalasi between '$tanggal_awal' and '$tanggal_akhir' order by barang_keluar.tgl_instalasi asc";	
}
