...
class FPDF_AutoWrapTable extends FPDF {
	...
	public function rptDetailData () {
		...
		$this->SetX($left += 60); $this->Cell(300, $h, 'NAMA BARANG', 1, 0, 'C',true);
		$this->SetX($left += 300); $this->Cell(80, $h, 'MERK', 1, 0, 'C',true);
		$this->SetX($left += 80); $this->Cell(300, $h, 'JENIS', 1, 0, 'C',true);
		$this->SetX($left += 300); $this->Cell(60, $h, 'STOCK', 1, 0, 'C',true);
		$this->Ln(20);
		
		$this->SetFont('Arial','',9);
		$this->SetWidths(array(30,60,300,80,300,60));
...
$query="select barang.*, jenis_barang.* from barang, jenis_barang where barang.jenis=jenis_barang.id_jenis and
$kategori LIKE '%$keyword%' order by id_barang";

}
else{
$query="select barang.*, jenis_barang.* from barang, jenis_barang where barang.jenis=jenis_barang.id_jenis order by id_barang";
