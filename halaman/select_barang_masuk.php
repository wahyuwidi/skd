...
<?php
include('../connect.php'); 
$sql = mysql_query("SELECT * FROM barang_masuk where stock>0 ORDER BY id_masuk");
?>
<html lang="en">
<head>
	<meta charset="utf-8">
	...
	<script>
		function sendValue (s,s2,s3,s4,s5,s6){

window.opener.document.getElementById('id_masuk').value = s;
...
window.opener.document.getElementById('tgl_masuk').value = s5;
window.opener.document.getElementById('id_barang').value = s6;
window.close();
}
	</script>
</head>
								...
								<table class="table table-bordered" id="datatable" >
									...
									<?php
										...
										<tr class='odd gradeX'>
											...
											$sql4 = mysql_query("SELECT*FROM supplier WHERE id_supplier = '$masuk[id_supplier]'");
											$supplier = mysql_fetch_array($sql4);//pecah hasil query ke dalam array
										...
										echo "<td>$jenis[nama_jenis]</td>
											  ...
											  <td class='center'>$masuk[stock]</td>"; ?>
											  <td><a href="#" onClick="sendValue('<?php echo $masuk['id_masuk']; ?>','<?php echo $barang['nama_barang']; ?>','<?php echo $barang['merk']; ?>','<?php echo $jenis['nama_jenis']; ?>','<?php echo $masuk['tgl_masuk']; ?>','<?php echo $barang['id_barang']; ?>');"><span class='btn btn-prusia'><i class='fa fa-check'></i>Pilih</span></a></td>
