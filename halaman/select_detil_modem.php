...
where detil_barang.id_barang=barang.id_barang and barang.jenis=jenis_barang.id_jenis and detil_barang.status='available'");
?>
...
<head>
	...
	<script>
		function sendValue (s,s2,s3,s4,s5){
	
			window.opener.document.getElementById('nama_barang').value = s;
			...
			window.opener.document.getElementById('id_detil').value = s5;
			window.close();
}
	</script>
	</script>
									...
									<?php
										...
										<tr class='odd gradeX'>
											...
											<td>$detil[nama_jenis]</td>"; ?>
											  <td><a href="#" onClick="sendValue('<?php echo $detil['nama_barang']; ?>','<?php echo $detil['mac_address']; ?>','<?php echo $detil['merk']; ?>','<?php echo $detil['nama_jenis']; ?>','<?php echo $detil['id_detil']; ?>');"><span class='btn btn-prusia'><i class='fa fa-check'></i>Pilih</span></a></td>
